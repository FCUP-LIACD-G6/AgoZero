...
class MuZeroConfig:
    def __init__(self):
        ...
        #self.action_space = [(i, j) for i in range(6) for j in range(6)]
...
class Game(AbstractGame):
    ...
    def legal_actions(self):
        ...
        Returns:
            An array of integers representing the entire action space.
        """
    ...
    def human_to_action(self):
        ...
        while True:
            source = input(f"Enter the source cell (row col) for the player {player}: ")
            destination = input(f"Enter the destination cell (row col) for the player {player}: ")
...
class Attaxx:
    ...
    def step(self, action):
        if not action or not isinstance(action, tuple) or len(action) != 2:
            ...
            print("Warning: Empty action received:",action)
