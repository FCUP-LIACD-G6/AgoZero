...
class MuZeroConfig:
    def __init__(self):
        ...
        self.observation_shape = (3, 5, 5)  # Dimensions of the game observation, must be 3D (channel, height, width). For a 1D array, please reshape it to (1, 1, length of array)
        board_size = 5
        ...
        self.training_steps = 3000000  # Total number of training steps (ie weights update according to a batch)
...
class Game(AbstractGame):
    ...
    # Current player mapped into 0/1
    def to_play(self):
    ...
    def legal_actions(self):
        legal_actions=self.env.legal_actions(self.env.player)
    ...
    def human_to_action(self):
        ...
        while True:
            ...
            if (source in self.legal_actions_source(player) and destination in self.legal_actions_dest(source)):
                ...
                # Encode action (source,dest) to move (int)
    ...
    # Display move (int) in cells (a1) form
...
class Attaxx:
    ...
    def __init__(self):
        self.size = 5
        self.board = numpy.zeros((5, 5), dtype="int32")
        self.board[0, 0] = 1
        self.board[-1, -1] = 1
        self.board[0, -1] = -1
        self.board[-1, 0] = -1
    ...
    # Current player mapped into pl-1=0/ pl1=1
    def to_play(self):
        return 1 if self.player == 1 else 0

    # Game iteration
    def step(self, action_int):
        # Terminate game if board id unplayable
        if self.board_has_moves() == False:
            reward = 1 if self.have_winner() else 0
            self.player *= -1
            return self.get_observation(), reward, True

        # Decode from move (int) to action (source,dest)
        action = self.decode(action_int)

        # Security against invalid actions (not used anymore)
    ...
    # Encodes action (source,destination) into move (int)
    ...
    # Decodes move (int) into action (source,destination)
    ...
    # Returns True if both players can perform action
    def board_has_moves(self):
        if self.legal_actions(1) == [] or self.legal_actions(-1) == []: return False
        elif self.empty_squares() == 0 or self.player_pieces(1) == 0 or self.player_pieces(-1) == 0: return False
        elif self.player_spots(1) == 0 or self.player_spots(-1) == 0: return False
        else: return True
    ...
    def game_over(self):
        ...
        if self.legal_actions(1) == [] or self.legal_actions(-1) == []: return True
        return False
    ...
    def legal_actions(self, player):
        ...
        for source in sources:
            ...
            for destination in destinations:
                action = (source,destination)
                # Encode action (source,dest) into move (int)
    ...
    def results(self):
        r = self.player_pieces(1)
        b = self.player_pieces(-1)
        return r,b