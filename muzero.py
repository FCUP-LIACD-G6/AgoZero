...
class MuZero:
    ...
    ### Function used to test our model against other agents
    def test_agent(self, render=True, opponent=None): #, ouragent=None, client_socket=None, Game=None):
        ...
        self_play_worker.play_game_agent.remote(0,0,render,opponent)#,ouragent,client_socket,Game)
...
# Connect to server to play against other agents
def connect_to_server(host='localhost', port=12345):
    ...
    first = False
    if "1" in response: first = True
    
    return client_socket, first, Game
...
if __name__ == "__main__":
    if len(sys.argv) == 2:
        ...
        while True:
            ...
            # New option to play adv game
            elif choice == 5:
                muzero.test_agent(render=True, opponent="agent")
            elif choice == 6:
                env = muzero.Game()
                env.reset()
                env.render()
